"""
AI Service — Multi-provider support for Zareb compliance analysis.
...
SYSTEM_PROMPT = """You are Zareb, an expert AI food safety compliance auditor with 20 years of experience auditing food manufacturers in Africa, EU, and UK markets. You specialize in HACCP, FSMA, SQF, BRCGS, ISO 22000, NAFDAC (Nigeria), KEBS (Kenya), and FDA EU regulations.
...
CHAT_SYSTEM_PROMPT = """You are Zareb, a food safety compliance expert. The user has just received a compliance report. Answer their questions clearly and specifically. If they ask about fixing a specific gap, give them exact template language they can use."""
        ...
        if provider == "groq":
            if os.getenv("GROQ_API_KEY") or settings.GROQ_API_KEY:
                try:
                    ...
                    print(f"[Zareb] AI provider: GROQ (groq.com - free)")
                    return
                except Exception as e:
                    print(f"[Zareb] Groq init failed: {e}")
            print("[Zareb] No GROQ_API_KEY set, trying Claude...")
        ...
        if provider == "claude" or not os.getenv("GROQ_API_KEY"):
            if os.getenv("ANTHROPIC_API_KEY") or settings.ANTHROPIC_API_KEY:
                try:
                    ...
                    print(f"[Zareb] AI provider: CLAUDE (Anthropic)")
                    return
                except Exception as e:
                    print(f"[Zareb] Claude init failed: {e}")

        # No API keys available — use mock provider
        print("[Zareb] WARNING: No AI API keys found. Using MOCK provider for development.")
        print("[Zareb] Set GROQ_API_KEY in .env for real AI analysis")
